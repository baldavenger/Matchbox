...
uniform int p_LMT;
uniform int p_RRT;
uniform int p_INVRRT;
uniform int p_ODT;
uniform int p_INVODT;
uniform int p_RRTODT;
uniform int p_INVRRTODT;
uniform float p_LMTScale1;
uniform float p_LMTScale2;
uniform float p_LMTScale3;
uniform float p_LMTScale4;
uniform float p_LMTScale5;
uniform float p_LMTScale6;
uniform float p_LMTScale7;
uniform float p_LMTScale8;
uniform float p_LMTScale9;
uniform float p_LMTScale10;
uniform float p_LMTScale11;
uniform float p_LMTScale12;
uniform float p_LMTScale13;
uniform float p_LMTScale14;
uniform float p_LMTScale15;
uniform float p_LMTScale16;
uniform float p_LMTScale17;
uniform float p_LMTScale18;
uniform float p_LMTScale19;
uniform float p_LMTScale20;
uniform float p_LMTScale21;
uniform float p_LMTScale22;
uniform float p_LMTScale23;
uniform float p_LMTScale24;
uniform float p_LMTScale25;
uniform float p_OT_Y_MIN;
uniform float p_OT_Y_MID;
uniform float p_OT_Y_MAX;
uniform int p_OT_Display;
uniform int p_OT_Limit;
uniform int p_OT_EOTF;
uniform int p_OT_SURR;
uniform bool p_OT_Stretch;
uniform bool p_OT_D60_SIM;
uniform bool p_OT_Legal;
uniform float p_InvOT_Y_MIN;
uniform float p_InvOT_Y_MID;
uniform float p_InvOT_Y_MAX;
uniform int p_InvOT_Display;
uniform int p_InvOT_Limit;
uniform int p_InvOT_EOTF;
uniform int p_InvOT_SURR;
...
{
Chromaticities OT_DISPLAY_PRI = p_OT_Display == 0 ? REC2020_PRI : p_OT_Display == 1 ? P3D60_PRI : p_OT_Display == 2 ? P3D65_PRI : p_OT_Display == 3 ? P3DCI_PRI : REC709_PRI;
Chromaticities OT_LIMIT_PRI = p_OT_Limit == 0 ? REC2020_PRI : p_OT_Limit == 1 ? P3D60_PRI : p_OT_Limit == 2 ? P3D65_PRI : p_OT_Limit == 3 ? P3DCI_PRI : REC709_PRI;
...
vec3 Custom_InvOutputTransform(vec3 aces, float Y_MIN, float Y_MID, float Y_MAX, int p_OT_Display, int p_OT_Limit, int EOTF, int SURROUND, bool STRETCH_BLACK, bool D60_SIM, bool LEGAL_RANGE)
{
Chromaticities InvOT_DISPLAY_PRI = p_InvOT_Display == 0 ? REC2020_PRI : p_InvOT_Display == 1 ? P3D60_PRI : p_InvOT_Display == 2 ? P3D65_PRI : p_InvOT_Display == 3 ? P3DCI_PRI : REC709_PRI;
Chromaticities InvOT_LIMIT_PRI = p_InvOT_Limit == 0 ? REC2020_PRI : p_InvOT_Limit == 1 ? P3D60_PRI : p_InvOT_Limit == 2 ? P3D65_PRI : p_InvOT_Limit == 3 ? P3DCI_PRI : REC709_PRI;
return invOutputTransform(aces, Y_MIN, Y_MID, Y_MAX, InvOT_DISPLAY_PRI, InvOT_LIMIT_PRI, EOTF, SURROUND, STRETCH_BLACK, D60_SIM, LEGAL_RANGE);
}
...
aces = ASCCDL_inACEScct(aces, SLOPE, OFFSET, POWER, p_LMTScale25);
}
if(p_LMTScale5 != 1.0)
aces = gamma_adjust_linear(aces, p_LMTScale5, p_LMTScale6);
aces = rgb_2_ych(aces);
if(p_LMTScale9 != 0.0)
aces = rotate_H_in_H_ych(aces, p_LMTScale7, p_LMTScale8, p_LMTScale9);
if(p_LMTScale12 != 0.0)
aces = rotate_H_in_H_ych(aces, p_LMTScale10, p_LMTScale11, p_LMTScale12);
if(p_LMTScale15 != 0.0)
aces = rotate_H_in_H_ych(aces, p_LMTScale13, p_LMTScale14, p_LMTScale15);
if(p_LMTScale18 != 1.0)
aces = scale_C_at_H_ych(aces, p_LMTScale16, p_LMTScale17, p_LMTScale18);
if(p_LMTScale21 != 0.0)
aces = rotate_H_in_H_ych(aces, p_LMTScale19, p_LMTScale20, p_LMTScale21);
if(p_LMTScale24 != 1.0)
aces = scale_C_at_H_ych(aces, p_LMTScale22, p_LMTScale23, p_LMTScale24);
aces = ych_2_rgb(aces);
return aces;
}
